...
class CommandController
{
    ...
    public function stopScream(StopScreamRequest $request, Device $device, StopScreamAction $action): JsonResponse { return response()->json(CommandResource::make($action->execute($request->user(), $device, $request->validated('alarm_secret'))), 201); }
    ...
    public function stolen(Request $request, Device $device, SetStolenModeAction $action): JsonResponse { return response()->json(['message' => 'Stolen mode activated', 'commands' => CommandResource::collection($action->execute($request->user(), $device))], 201); }
    public function found(FoundDeviceRequest $request, Device $device, SetFoundModeAction $action): JsonResponse { return response()->json(\App\Presentation\Http\Resources\DeviceResource::make($action->execute($request->user(), $device, $request->validated('pin_code'))), 200); }
